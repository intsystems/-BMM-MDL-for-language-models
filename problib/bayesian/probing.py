...
class BayesianProbingModel(BaseModel):
    ...
    def evaluate(self, evalloader: torch.utils.data.DataLoader, model) -> Dict[str, float]:
        """
        ...
        with torch.no_grad():  # Disable gradient calculation for evaluation
            ...
            n_instances = len(evalloader.dataset)  # Total number of instances in evaluation dataset
    ...
    def train_epoch(self, trainloader: torch.utils.data.DataLoader, devloader: torch.utils.data.DataLoader,
                    model, optimizer: optim.Optimizer, train_info: TrainInfo):
        """
        ...
        for x, y in trainloader:
            ...
            if train_info.eval:
                dev_results = self.evaluate(devloader, model)  # Evaluate on development set

                if train_info.is_best(dev_results):
                    model.set_best()  # Mark current model as best if results improve
                
                elif train_info.finish:
                    train_info.print_progress(dev_results)  # Print progress if training is finished
                    return

                train_info.print_progress(dev_results)  # Print current evaluation results

    def train(self, trainloader: torch.utils.data.DataLoader, devloader: torch.utils.data.DataLoader,
              model, eval_batches: int, wait_iterations: int):
        """
        ...
        with tqdm(total=wait_iterations) as pbar:
            train_info = TrainInfo(pbar, wait_iterations, eval_batches)  # Initialize training info tracker
            
            while not train_info.finish:
                self.train_epoch(trainloader, devloader, model, optimizer, train_info)  # Train for one epoch
