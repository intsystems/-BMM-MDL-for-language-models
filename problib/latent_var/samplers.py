...
class BaseSampler(nn.Module):
    ...
    def get_top_k_features(self) -> List[int]:
        raise NotImplementedError(
            "Each sampler must implement the get_top_k_features method."
        )
...
class ConditionalPoissonSampler(BaseSampler):
    ...
    def entropy(self, cache: Optional[List[torch.Tensor]] = None) -> torch.Tensor:
        return self.run_on_semiring(
            torch_struct.EntropySemiring, cache if cache is not None else []
        )

    def logZ(self, cache: Optional[List[torch.Tensor]] = None) -> torch.Tensor:
        return self.run_on_semiring(
            torch_struct.LogSemiring, cache if cache is not None else []
        )

    def logprob(
        self, mask: torch.Tensor, logZ: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        if logZ is None:
            logZ = self.logZ()
    ...
    def sample(self, num_samples: int, log_cache: List[torch.Tensor]) -> torch.Tensor:
        num_to_sample = self.K * torch.ones(
            num_samples, device=self.get_device(), dtype=torch.long
        )
    ...
    def _sample(
        self,
        num_samples: int,
        num_to_sample: torch.Tensor,
        log_cache: List[torch.Tensor],
    ) -> torch.Tensor:
        samples = torch.zeros(
            num_samples, self.D, device=self.weights.device, dtype=torch.long
        )
    ...
    def _get_from_cache(
        self, N: int, n: int, cache: List[torch.Tensor]
    ) -> torch.Tensor:
        return cache[N - 1][..., n]

    def _compute_S_N_n(
        self, sr, weights: torch.Tensor, N: int, n: int, cache: List[torch.Tensor]
    ) -> torch.Tensor:
